...
public class TestSuit {
    ...
    [UnityTest]
    public IEnumerator HorizontallyMatchingTilesAreRemoved()
    {
        GameObject gameObject = new GameObject();
    ...
    [UnityTest]
    public IEnumerator OnlyHorizontallyMatchingTilesAreRemoved()
    {
        GameObject gameObject = new GameObject();

        GameField gameField = gameObject.AddComponent<GameField>();
        gameField.SetTestData(3, 5, new GameObject("Field"), new GameObject("Background"));//ToDo - swap width and height parameters around to match rest of Logic and Array logic

        GameLogic gameLogic = gameObject.AddComponent<GameLogic>();

        GameObject[,] tilesOnField = new GameObject[3, 5] {
                        { new GameObject("A"), new GameObject("B"), new GameObject("C"), new GameObject("D"), new GameObject("A")},  // 0,0   0,1   0,2   0,3   0,4
                        { new GameObject("B"), new GameObject("A"), new GameObject("A"), new GameObject("D"), new GameObject("A")},  // 1,0   1,1   1,2   1,3   1,4
                        { new GameObject("A"), new GameObject("B"), new GameObject("B"), new GameObject("C"), new GameObject("A")} };// 0,2   1, 2   2, 2

        gameLogic.SetTestData(gameField, tilesOnField);

        gameLogic.RemoveTiles(1, 0); //removing B from 2nd row start to get match of 3 x A on that row

        //Top row
        Assert.IsNull(tilesOnField[0, 0]);
        Assert.IsNull(tilesOnField[0, 1]);
        Assert.IsNull(tilesOnField[0, 2]);
        Assert.AreEqual("D", tilesOnField[0, 3].name);
        Assert.AreEqual("A", tilesOnField[0, 4].name);

        //Center row (where 3 x A matched)
        Assert.IsNull(tilesOnField[1, 0]);
        Assert.AreEqual("B", tilesOnField[1, 1].name);
        Assert.AreEqual("C", tilesOnField[1, 2].name);
        Assert.AreEqual("D", tilesOnField[1, 3].name);
        Assert.AreEqual("A", tilesOnField[1, 4].name);

        //Bottom row
        Assert.AreEqual("A", tilesOnField[2, 0].name);
        Assert.AreEqual("B", tilesOnField[2, 1].name);
        Assert.AreEqual("B", tilesOnField[2, 2].name);
        Assert.AreEqual("C", tilesOnField[2, 3].name);
        Assert.AreEqual("A", tilesOnField[2, 4].name);
    ...
    public IEnumerator TilesFallDown()
    {
        ...
        GameField gameField = gameObject.AddComponent<GameField>();
        gameField.SetTestData(3, 1, new GameObject("Field"), new GameObject("Background"));
